...
## computes changes in land use on a 2km basis from 2000 to 2031, focussing on
...
urban_sprawl <- stack('TotalUrbanAndSuburban_2031_HighDensityScenario_1km.tif')
...
sprawl_2km <- st_join(seer_2km, sprawl_ctrds) %>% 
  group_by(new2kid) %>% 
  summarise(mean = mean(TotalUrbanAndSuburban_2031_HighDensityScenario_1km))
...
setwd('D:/Documents/Housing/Output')
st_write(seer_2km, 'seer_landuses_densification_2000.csv')
...
setwd('D:/Documents/Housing/Output')
st_write(seer_2km, 'seer_landuses_densification_2031.csv')
